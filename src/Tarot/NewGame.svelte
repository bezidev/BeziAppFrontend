<script lang="ts">
    ...
    async function newGame() {
        ...
        await makeRequest(`/tarot/contest/${id}`, "POST", JSON.stringify({
...
{#if contest !== undefined}
    <h1>Nova igra</h1>
    <p/>
    <SegmentedButton segments={Object.keys(GAMEMODES)} let:segment singleSelect bind:selected={gamemode}><Segment {segment}><Label>{segment}</Label></Segment></SegmentedButton>
    ...
    <Select bind:value={contestant} label="Igralci">
        {#each JSON.parse(contest.contestants) as contestant}
            <Option value={contestant} on:click={() => {
                ...
                if (ok) {
                    contestants.push({username: contestant, difference: 0, playing: false});
    ...
    {#each contestants as cont}
        ...
        <Button on:click={async () => {
            ...
            for (let i in contestants) {
                if (contestants[i].username !== cont.username) cs.push(contestants[i]);
            }
            contestants = cs;
        }} variant="raised">
